...
export async function fetchClassDetail(id: number): Promise<ClassDetail> {
  const res = await apiFetch(apiUrl(`/api/classes/${id}`), { headers: authHeaders() });
  if (!res.ok) throw new Error("LOAD_FAILED");
  return res.json();
}

export async function createClass(input: ClassInput): Promise<ClassSummary> {
  const res = await apiFetch(apiUrl("/api/classes"), {
    method: "POST",
    headers: authHeaders(),
...
export async function updateClass(id: number, input: ClassInput): Promise<ClassSummary> {
  const res = await apiFetch(apiUrl(`/api/classes/${id}`), {
    method: "PUT",
    headers: authHeaders(),
    body: JSON.stringify(input),
  });
...
export async function setClassActive(id: number, active: boolean): Promise<ClassSummary> {
  const res = await apiFetch(apiUrl(`/api/classes/${id}/active`), {
    method: "PATCH",
    headers: authHeaders(),
...
export async function addStudentToClass(classId: number, studentId: number): Promise<ClassDetail> {
  const res = await apiFetch(apiUrl(`/api/classes/${classId}/students/${studentId}`), {
    method: "POST",
    headers: authHeaders(),
  });
  if (!res.ok) {
