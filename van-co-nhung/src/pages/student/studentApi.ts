...
export async function fetchMySchedule(): Promise<MySchedule> {
  const res = await apiFetch(apiUrl("/api/me/schedule"), { headers: authHeaders() });
  if (!res.ok) throw new Error("LOAD_FAILED");
  return res.json();
}

export async function fetchMyClassmates(): Promise<MyClassmates> {
  const res = await apiFetch(apiUrl("/api/me/classmates"), { headers: authHeaders() });
  if (!res.ok) throw new Error("LOAD_FAILED");
  return res.json();
}

export async function fetchMyTeacher(): Promise<TeacherContact | null> {
  const res = await apiFetch(apiUrl("/api/me/teacher"), { headers: authHeaders() });
